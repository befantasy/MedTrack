...
【重要核心要求】：
1. 提炼的 ai_summary 必须严谨、专业，按顺序包含以下三部分（客观总结陈述 + 异常指标提示 + 建议）：
   - ①【客观总结陈述】：对化验单整体情况进行客观综合评价（例如：“本次检验涵盖生化全套与血糖血脂指标，总体评估代谢与脂质水平存在偏高，肾功能与电解质保持正常。”；若全部指标均在正常范围，则直接写为：“各项检验指标均在正常参考范围内，未见异常波动。”）。
   - ②【异常指标提示】：若存在任何“偏高(↑)”、“偏低(↓)”或“异常/阳性”指标，必须以“异常指标: ”明确开头，逐一列出：指标中文全称及英文代码、具体实测测定值、计量单位、参考区间以及偏高/偏低判定。例如：“异常指标: 葡萄糖(GLU) 9.14 mmol/L (↑偏高, 参考:3.89-6.11)、总胆固醇(CHOL) 7.06 mmol/L (↑偏高, 参考:0-5.17)、低密度脂蛋白胆固醇(LDL-C) 5.54 mmol/L (↑偏高, 参考:0-3.4)；其余指标均在正常参考范围内。”。必须包含具体的测定数值与参考区间，严禁仅使用泛泛概括！若全项指标均正常，则可省略此项或注明“未见异常指标”。
   - ③【建议】：必须以“建议: ”明确开头，结合上述异常指标与临床实际，给出科学实用的医学复查、就医随访或生活方式干预建议（例如：“建议: 定期监测空腹血糖与血脂谱，低糖低脂清淡饮食，必要时门诊复查评估。”）。
...
请严格按以下 JSON 结构输出，不要包含任何 markdown 之外的解释：
{
  ...
  "hospital": "医院名称",
  "ai_summary": "客观总结陈述。 异常指标: 指标名(代码) 数值 单位 (↑偏高/↓偏低, 参考:区间)；其余指标均在正常参考范围内。 建议: 具体随访与就医建议。",
  "items": [
    {
